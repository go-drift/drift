...
final class PlatformChannelManager: DriftPluginHost {
    ...
    /// method so the dozens of in-tree callers (`sendEvent(channel:data:)`)
    /// continue to work without a sweeping rename.
    func sendEvent(_ channel: String, data: Any?) {
        sendEvent(channel: channel, data: data)
    }

    /// `DriftPluginHost`-shaped error sender. Mirrors `sendEvent(_:data:)`.
    func sendEventError(_ channel: String, code: String, message: String) {
        sendEventError(channel: channel, code: code, message: message)
    }

    /// `DriftPluginHost`-shaped done sender. Mirrors `sendEvent(_:data:)`.
    ...
    private func registerBuiltInChannels() {
        ...
        // Third-party plugins. DriftPluginRegistrant is generated by the
        // Drift plugin pipeline; the body is empty when no plugins are
        // configured.
        DriftPluginRegistrant.registerAll(host: self)
    }
}
