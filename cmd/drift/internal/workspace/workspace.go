...
// the absolute path to the current drift binary. This file is sourced by
// driftw to find drift without relying on PATH (which is minimal in IDE
// build environments like Xcode). The file is machine-specific and should
// be gitignored.
func WriteDriftEnv(dir string) error {
	...
	content := fmt.Sprintf("# Auto-generated by drift. Do not commit.\nDRIFT_BIN=%q\n", exe)
	return os.WriteFile(filepath.Join(dir, ".drift.env"), []byte(content), 0o644)
}
