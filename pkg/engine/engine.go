...
func (a *appRunner) Paint(canvas graphics.Canvas, size graphics.Size) (err error) {
	...
	if a.rootRender != nil {
		...
		// Phase 1: Record dirty layers
		// Only traverse subtrees containing dirty boundaries to avoid full-tree walks
...
// Uses the pre-computed dirty list to avoid full-tree traversal every frame.
// For each dirty boundary, we ensure all descendant boundaries are recorded first
// (children before parents) so DrawChildLayer ops reference valid content.
...
func recordDirtyLayersDFS(node layout.RenderObject, showLayoutBounds bool, strokeWidth float64, isRoot bool) {
	...
	// We check RepaintBoundaryNode separately from ChildVisitor so that leaf
	// boundaries (like renderImage/renderSvgImage) that don't have children
	// are still detected and recorded properly.
	var isBoundary bool
	var needsPaint bool
	...
	// This is separate from the boundary check since leaf boundaries don't have children.
