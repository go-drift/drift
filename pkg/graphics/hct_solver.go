...
// criticalPlanes contains 256 L* values used for gamut boundary detection.
var criticalPlanes = [256]float64{
...
// solveToColor finds the sRGB color closest to the given HCT coordinates.
func solveToColor(hueDegrees, chroma, lstar float64) Color {
	...
	y := yFromLstar(lstar)
	if exact, ok := findResultByJ(hueRadians, chroma, y); ok {
		return exact
	}
...
// colorFromLstar returns the grayscale Color matching the given L*.
...
// colorFromLinrgb converts linear RGB [0-100] to a Color.
func colorFromLinrgb(linrgb [3]float64) Color {
	return RGB(delinearized(linrgb[0]), delinearized(linrgb[1]), delinearized(linrgb[2]))
}

func findResultByJ(hueRadians, chroma, y float64) (Color, bool) {
	j := math.Sqrt(y) * 11.0
	tInnerCoeff := 1.0 / math.Pow(1.64-math.Pow(0.29, vcN), 0.73)
	eHue := 0.25 * (math.Cos(hueRadians+2.0) + 3.8)
	p1 := eHue * (50000.0 / 13.0) * vcNc * vcNcb
	...
	for i := range 5 {
		...
		ac := vcAw * math.Pow(jNorm, 1.0/vcC/vcZ)
		p2 := ac / vcNbb
		...
		linrgb := matrixMultiply([3]float64{rCScaled, gCScaled, bCScaled}, linrgbFromScaledDiscount)
		...
		if i == 4 || math.Abs(fnj-y) < 0.002 {
			...
			return colorFromLinrgb(linrgb), true
		}
		j = j - (fnj-y)*j/(2*fnj)
	}
	return 0, false
...
func bisectToSegment(y float64, targetHue float64) ([3]float64, [3]float64) {
...
func bisectToLimit(y float64, targetHue float64) [3]float64 {
	...
	for axis := range 3 {
		if left[axis] != right[axis] {
			...
			for range 8 {
				...
				midCoord := criticalPlanes[mPlane]
				mid := setCoordinate(left, midCoord, right, axis)
				midHue := hueOf(mid)
				if areInCyclicOrder(leftHue, targetHue, midHue) {
					right = mid
					rPlane = mPlane
...
func hueOf(linrgb [3]float64) float64 {
	scaled := matrixMultiply(linrgb, scaledDiscountFromLinrgb)
	rA := chromaticAdaptation(scaled[0])
	gA := chromaticAdaptation(scaled[1])
	bA := chromaticAdaptation(scaled[2])
...
func criticalPlaneBelow(x float64) int {
	return max(int(math.Floor(x-0.5)), 0)
}

func criticalPlaneAbove(x float64) int {
	return min(int(math.Ceil(x-0.5)), 255)
}

func matrixMultiply(row [3]float64, matrix [3][3]float64) [3]float64 {
	return [3]float64{
